...
app.addListeners = () => {
  app.filters.forEach((item) => {
    item.onclick = () => {
      ...
      item.innerHTML =
        '<span class="material-symbols-rounded">done</span>' + item.innerHTML;
    };
  });
};
...
app.useCurrentDate = (skip) => {
  ...
  app.offset =
    parseInt(local.split(" ")[1].split(":")[0]) -
    parseInt(req.split(" ")[1].split(":")[0]);
  ...
  for (let i = app.events.length - 1; i >= 0; i--) {
    if (
      app.events[i].dataset.start <= d.getHours() + d.getMinutes() / 60 &&
      app.events[i].dataset.end > d.getHours() + d.getMinutes() / 60
    ) {
      app.events[i].classList.add("current");
      if (!skip)
        document.getElementById("rightNow").innerText =
          app.events[i].querySelector(".card-title").innerText;
      return;
    }
  }
};
...
app.updateView = (day) => {
  ...
  for (let i = 1; i <= Object.keys(app.classes[day]).length; i++) {
    ...
    if (app.classes[day][i]["busy"]) el.classList.add("busy");
    let startNum =
      parseInt(app.classes[day][i]["start"].split(":")[0]) +
      parseInt(app.classes[day][i]["start"].split(":")[1]) / 60;
    let endNum =
      parseInt(app.classes[day][i]["end"].split(":")[0]) +
      parseInt(app.classes[day][i]["end"].split(":")[1]) / 60;
    startNum += app.offset;
    endNum += app.offset;
    ...
    if (Math.floor(startNum) == 0 || Math.floor(startNum) == 12) startNum += 12;
    if (Math.floor(endNum) == 0 || Math.floor(endNum) == 12) endNum += 12;
    const start =
      startNum > 12
        ? Math.floor(startNum) -
          12 +
          ":" +
          String((startNum - Math.floor(startNum)) * 60)
            .padStart(2, "0")
            .split(".")[0] +
          " PM"
        : Math.floor(startNum) +
    ...
    const end =
      endNum > 12
        ? Math.floor(endNum) -
          12 +
          ":" +
          String((endNum - Math.floor(endNum)) * 60)
            .padStart(2, "0")
            .split(".")[0] +
          " PM"
        : Math.floor(endNum) +
